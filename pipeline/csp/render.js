...
const C = {
  ...
  primaryAlt: '10B981',  // bright emerald
  bgTint:     'ECFDF5',  // very light emerald background
  cardTint:   'F0FDF4',  // card surface
  dark:       '064E3B',  // dark emerald ink (titles, dividers)
  darker:     '022C22',  // even darker for title slide
  body:       '1F2937',  // body text
  muted:      '6B7280',  // muted text
  white:      'FFFFFF',
  amber:      'F59E0B',  // accent for callouts (warm-up / bell ringer circles)
  amberSoft:  'FEF3C7',
  teal:       '0E7490',  // cyan accent: site / practice circles + Your Turn slide
  tealSoft:   'CFFAFE',
  tealDeep:   '155E75',
  redSoft:    'FEE2E2',
  red:        'DC2626',
...
function buildTitle(slide, s) {
  ...
  if (s.subtitle) {
    slide.addText(s.subtitle, {
      ...
      fontFace: F.body, fontSize: 18, color: 'A7F3D0', italic: true,
      align: 'left', valign: 'top', margin: 0,
    });
  }
  ...
  slide.addText(s.footerLeft || `${meta.course} \u00b7 CB V.1 Framework`, {
    ...
    fontFace: F.body, fontSize: 11, color: 'A9C8BB', align: 'left', margin: 0,
  });
  slide.addText(s.footerRight || 'APCSExamPrep.com', {
    x: W - 4 - M.edge - 0.5, y: 6.2, w: 4, h: 0.3,
    fontFace: F.body, fontSize: 11, color: 'A9C8BB', align: 'right', margin: 0, bold: true,
  });
...
function buildSectionDivider(slide, s) {
  ...
  if (s.kicker) {
    slide.addText(s.kicker, {
      ...
      fontFace: F.body, fontSize: 14, italic: true, color: 'A9C8BB',
      align: 'left', margin: 0,
    });
  }
}
  ...
  [s.left, s.right].forEach((col, i) => {
    ...
    if (col.ek) {
      slide.addText(col.ek, {
        ...
        fontFace: F.body, fontSize: 10, color: 'D1FAE5', italic: true, align: 'left', margin: 0,
      });
    }
    // Definition
...
function buildDiscussion(slide, s) {
  ...
  if (s.subprompt) {
    slide.addText(s.subprompt, {
      ...
      fontFace: F.body, fontSize: 18, italic: true, color: 'A7F3D0', align: 'left', valign: 'top', margin: 0,
    });
  }
...
function buildWorkTimer(slide, s) {
  ...
  slide.addText(s.durationLabel || 'MINUTES', {
    ...
    fontFace: F.body, fontSize: 14, bold: true, color: 'A9C8BB',
    align: 'left', margin: 0,
  });
...
function buildScenarioIntro(slide, s) {
  ...
  slide.addText(s.narrative, {
    ...
    fontFace: F.body, fontSize: 17, color: 'D6EAE0',
  ...
  if (s.job) {
    ...
    slide.addText(s.job, {
      ...
      fontFace: F.body, fontSize: 16, color: 'D6EAE0', italic: true,
